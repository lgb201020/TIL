...
- 예시) 선수과목을 고려한 학습 순서 설정
  ...
  ![status_range_1](./img/status_range_1.png)
...
- 진출차수(Outdegree): 특정한 노드에서 나가는 간선의 개수
  ...
  ![status_range_2](./img/status_range_2.png)

### 위상 정렬 알고리즘
- 위상 정렬은 DFS를 이용하거나 큐를 이용해서 구현이 가능하다.
- **큐를 이용**하는 위상 정렬 알고리즘의 동작 과정은 다음과 같다.
...
- 위상 정렬을 수행할 그래프를 준비한다.
  ...
  - 사이클이 있는 경우 모든 노드의 진입차수가 1 이상이 되므로 위상 정렬을 수행할 수 없다.

  ![status_range_3](./img/status_range_3.png)

- [초기 단계] 초기 단계에서는 **진입차수가 0인 모든 노드**를 큐에 넣는다.
  - 처음에 **노드 1**이 큐에 삽입된다.

  ![status_range_4](./img/status_range_4.png)

- [Step 1] 큐에서 **노드 1**을 꺼낸 뒤에 **노드 1**에서 나가는 간선을 제거한다.
  - 새롭게 **진입차수가 0이 된 노드**들을 큐에 삽입한다.

  ![status_range_5](./img/status_range_5.png)

- [Step 2] 큐에서 **노드 2**를 꺼낸 뒤에 **노드 2**에서 나가는 간선을 제거한다.
  - 새롭게 **진입차수가 0이 된 노드**들을 큐에 삽입한다.
  - 한 번의 스텝에서 여러개의 노드가 큐에 들어갈 경우 어떤 순서대로 원소를 큐에 넣어도 상관없으나 일반적으로 더 작은 번호의 노드를 우선적으로 넣는다.

  ![status_range_6](./img/status_range_6.png)

- [Step 3] 큐에서 **노드 5**를 꺼낸 뒤에 **노드 5**에서 나가는 간선을 제거한다.
  - 새롭게 **진입차수가 0이 된 노드**들을 큐에 삽입한다.

  ![status_range_7](./img/status_range_7.png)

- [Step 4] 큐에서 **노드 3**을 꺼낸 뒤에 **노드 3**에서 나가는 간선을 제거한다.
  - 새롭게 **진입차수가 0이 된 노드**가 없으므로 그냥 넘어간다.

  ![status_range_8](./img/status_range_8.png)

- [Step 5] 큐에서 **노드 6**을 꺼낸 뒤에 **노드 6**에서 나가는 간선을 제거한다.
  - 새롭게 **진입차수가 0이 된 노드**들을 큐에 삽입한다.

  ![status_range_9](./img/status_range_9.png)

- [Step 6] 큐에서 **노드 4**를 꺼낸 뒤에 **노드 4**에서 나가는 간선을 제거한다.
  - 새롭게 **진입차수가 0이 된 노드**를 큐에 삽입한다.

  ![status_range_10](./img/status_range_10.png)

- [Step 7] 큐에서 **노드 7**를 꺼낸 뒤에 **노드 7**에서 나가는 간선을 제거한다.
  - 새롭게 **진입차수가 0이 된 노드**가 없으므로 그냥 넘어간다.

  ![status_range_11](./img/status_range_11.png)

- [위상 정렬 결과] 큐에 삽입된 전체 노드 순서: 1 -> 2 -> 5 -> 3 -> 6 -> 4 -> 7

  ![status_range_11 (1)](./img/status_range_11(1).png)

### 위상 정렬의 특징
- 위상 정렬은 DAG에 대해서만 수행할 수 있다.
  - **DAG(Direct Acyclic Graph):** 순환하지 않는 방향 그래프

- 위상 정렬에서 **여러 가지 답이 존재**할 수 있다.
...
# 위상 정렬 함수
def topology_sort():
  result = [] # 위상 정렬 알고리즘 수행 결과를 담을 리스트
  q = deque()
  ...
  while q:
    ...
    # 해당 원소와 연결된 노드들의 진입차수에서 1 빼기(now에서 나가는 간선 삭제)
    for i in graph[now]:
      indegree[i] -= 1
