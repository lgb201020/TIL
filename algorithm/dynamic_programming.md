...
- 병사를 배치할 때는 **전투력이 높은 병사가 앞쪽에 오도록 내림차순으로 배치**를 하고자 한다. 다시 말해 앞쪽 병사의 전투력이 항상 뒤쪽 병사보다 높아야 한다.
...
- 이때 3번 병사와 6번 병사를 열외시키면, 다음과 같이 남아 있는 병사의 수가 내림차순의 형태가 되며 5명이 된다. 이는 남아 있는 병수의 수가 최대가 되도록 하는 방법이다.

  |병사 번호|1|2|4|5|7|
  |--------|-|-|-|-|-|
  |전투력|15|11|8|5|4|

- 병사에 대한 정보가 주어졌을 때, 남아 있는 병사의 수가 최대가 되도록 하기 위해 열외시켜야 하는 병사의 수를 출력하는 프로그램을 작성하시오.

### 문제 조건
![soldier_3](./img/soldier_3.png)

### 문제 해결 아이디어
- 이 문제의 기본 아니디어는 **가장 긴 증가하는 부분 수열(LIS)**로 알려진 전형적인 다이나믹 프로그래밍 문제의 아이디어와 같다.

- 예를 들어 하나의 수열 array = {4, 2, 5, 8, 4, 11, 15}이 있다고 하자.
  - 이 수열의 가장 긴 증가하는 부분 수열은 {4, 5, 8, 11, 15}이다.

- 본 문제는 가장 긴 감소하는 부분 수열을 찾는 문제로 치환할 수 있으므로, LIS 알고리즘을 조금 수정하여 적용함으로써 해결 할 수 있다.

- [가장 긴 증가하는 부분 수열(LIS) 알고리즘](https://github.com/lgb201020/TIL/blob/main/algorithm/LIS_algorithm.md)

- 병사 배치하기 문제는 가장 긴 감소하는 부분 수열을 구하는 문제이다.
  - 가장 먼저 입력 받은 병사 정보의 순서를 뒤집는다.
  - LIS 알고리즘을 수행하여 문제에서 요구하는 내림차순이면서 남아 있는 병수의 수가 최대가 되도록 하는 배열 길이를 얻을 수 있다. 
