# 문자열과 튜플

## 문자열 자료형
- 문자열 변수를 초기화할 때는 큰따옴표(")나 작은 따옴표(')를 사용한다.
  - 전체 문자열을 큰따옴표로 구성하는 경우 내부적으로 작은따옴표를 포함할 수 있으며 전체 문자열을 작은따옴표로 구성하는 경우 내부적으로 큰따옴표를 포함할 수 있다.
  - 백슬래시를 사용하면 큰따옴표나 작은따옴표를 원하는 만큼 포함시킬 수 있다.
    ```
    data1 = 'Hello World'
    data2 = "Don't you know \"Python\"?"

    print(data1) # Hello World
    print(data2) # Don't you know "Python"?
    ```

## 문자열 연산
- 문자열 변수에 덧셈(+)을 이용하면 문자열이 더해져서 연결된다.
- 문자열 변수를 특정 양의 정수와 곱하는 경우, 문자열이 그 값만큼 여러 번 반복해서 더해진다.

- 문자열에 대해서도 마찬가지로 인덱싱과 슬라이싱을 이용할 수 있다.
  - 다만 문자열은 특정 인덱스의 값을 변경할 수 없다.(immutable)
  ```
  a = "Hello"
  b = "World"
  print(a + " " + b) # Hello World

  a = "String"
  print(a * 3) # StringStringString

  a = "ABCDEF"
  print(a[2 : 4]) # CD
  ```

## 문자열관련 method
- in 연산자: 문자열에서도 리스트와 동일하게 in 연산자를 사용 가능하다.
  - str2 in str1과 같이 str1 문자열 안에 str2문자열이 있다면 True, 없다면 False를 반환할 것이다.
  - str1 = "ab`6CD`E443fgh22iJKlmn1o", str2 = "6CD"	일 때, str2 in str1는 True를 반환한다.

- replace 메서드: 문자열.replace(타겟 문자, 대체할 문자)형식으로 사용하며 문자열 내 타겟 문자를 다른 문자로 대체할 때 쓴다.
  - 이때 vowels = ['a','e','i','o','u'], my_string = my_string.replace(vowel, '')로 써도 가능하다.

- 문자열 확인 및 변환 메서드
  - 문자열.isupper() / 문자열.islower() / 문자열.isdigit():각각 대문자이면 True, 소문자이면 True, 숫자로 된 문자열이면 True를 반환한다.
  - 문자열.upper() / 문자열.lower(): 각각 문자열을 대문자, 소문자로 바꿔주는 메서드 이다.
  - 문자열.swapcase(): 문자열이 소문자이면 대문자로, 대문자이면 소문자로 변환하는 메서드이다.


## 튜플 자료형
- 튜플 자료형은 리스트와 유사하나 다음과 같은 문법적 차이가 있다.
  - ***튜플은 한 번 선언된 값을 변경할 수 없다.***
  - 리스트는 대괄호([])를 사용하지만 튜플은 소괄호(())를 사용한다.  
  - 튜플은 리스트에 비해 상대적으로 공간 효율적이다.
  ```
  a = (1, 2, 3, 4, 5, 6, 7, 8, 9)

  # 네 번째 원소만 출력 
  print(a[3]) # 4
  # 두 번째 원소부터 네 번째 원소까지 출력
  print(a[1:4]) # (2, 3, 4)

  a[1] = 10 # 이 경우 오류가 발생
  ```
- 튜플을 사용하면 좋은 경우는 다음과 같다.
  - `서로 다른 성질의 데이터를 묶어 관리할 때`, 최단 경로 알고리즘에서는 (비용, 노트 번호)의 형태로 튜플 자료형을 자주 사용한다.
  - 데이터의 나열을 `해싱(Hashing)의 키 값으로 사용해야 할 때`, 튜플은 변경이 불가능하므로 리스트와 다르게 키 값으로 사용될 수 있다.
  - 리스트보다 `메모리를 효율적으로 사용해야 할 때`